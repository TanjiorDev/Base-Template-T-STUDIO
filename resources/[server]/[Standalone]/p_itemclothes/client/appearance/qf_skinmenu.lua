...
if Config.Appearance ~= "qf_skinmenu" then
    return
end
...
function Skin.updateClothes(self)
    local pedAppearance = exports.qf_skinmenu:getPedAppearance(cache.ped)
    local clothesPayload = {}
    local convertedSkin = self:convert({
        props = pedAppearance.props,
        components = pedAppearance.components,
    })
    ...
    if Bridge and Bridge.Config and Bridge.Config.Debug then
        ...
        lib.print.info("Current skin:", pedAppearance.props, pedAppearance.components)
...
function Skin.saveSkin(self)
    ...
    TriggerServerEvent(
        ...
        exports.qf_skinmenu:getPedAppearance(cache.ped)
    )
end
...
AddEventHandler("ox_inventory:updateInventory", function(inventoryChanges)
    ...
    if hasAppearanceChanges then
        exports.qf_skinmenu:setPedComponents(cache.ped, componentsToSet)
        exports.qf_skinmenu:setPedProps(cache.ped, propsToSet)
        Citizen.Wait(1)
        Ped:update()
        Skin:saveSkin()
    end
...
AddStateBagChangeHandler("InSkin", nil, function(bagName, _, inSkin)
    local playerId = GetPlayerFromStateBagName(bagName)
    if not playerId or playerId == 0 then
        return
    end
    if GetPlayerPed(playerId) ~= cache.ped then
        return
    end
    if not inSkin then
        Citizen.Wait(150)
