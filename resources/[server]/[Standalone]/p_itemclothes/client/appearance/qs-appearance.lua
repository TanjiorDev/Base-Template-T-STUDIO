...
if Config.Appearance ~= "qs-appearance" then
    return
end
...
function Skin.updateClothes(self)
    ...
    local pedProps = exports["qs-appearance"]:getPedProps(cache.ped)
    local pedComponents = exports["qs-appearance"]:getPedComponents(cache.ped)
    ...
    if Bridge and Bridge.Config and Bridge.Config.Debug then
        lib.print.info("player gender", playerSex)
        lib.print.info("player current skin", convertedSkin)
    end
    ...
    if Bridge and Bridge.Config and Bridge.Config.Debug then
        lib.print.info("clothes items data", clothesPayload)
    end
...
function Skin.saveSkin(self)
    ...
    TriggerServerEvent(
        ...
        exports["qs-appearance"]:getPedAppearance(cache.ped)
    )
end
...
AddEventHandler("ox_inventory:updateInventory", function(inventoryChanges)
    ...
    if hasAppearanceChanges then
        exports["qs-appearance"]:setPedComponents(cache.ped, componentsToSet)
        exports["qs-appearance"]:setPedProps(cache.ped, propsToSet)
        Citizen.Wait(1)
        Ped:update()
        Skin:saveSkin()
    end
