...
---@class ESXJob2
---@field id number                 # Job2 ID.
---@field name string               # Job2 internal name.
---@field label string              # Job2 display label.
---@field grade number              # Current grade/rank number.
---@field grade_name string         # Name of the current grade.
---@field grade_label string        # Label of the current grade.
---@field grade_salary number       # Salary for the current grade.
...
---@field getJob2 fun(): ESXJob2                                       # Get player's current job2.
---@field setJob2 fun(newJob2: string, grade: string, onDuty?: boolean) # Set player's job2 and grade.
...
---@field job2 ESXJob2              # Player's secondary job.
...
function CreateExtendedPlayer(playerId, identifier, ssn, group, accounts, inventory, weight, job, job2, loadout, name, coords, metadata)
    ...
    if type(self.metadata.job2Duty) ~= "boolean" then
        self.metadata.job2Duty = self.job2.name ~= "unemployed" and Config.DefaultJob2Duty or false
    end
    ...
    function self.setJob2(newJob2, grade, onDuty)
        ...
        if not ESX.DoesJob2Exist(newJob2, grade) then
        ...
        if type(onDuty) ~= "boolean" then
            onDuty = Config.DefaultJob2Duty
        end

        local job2Object, gradeObject = ESX.Jobs2[newJob2], ESX.Jobs2[newJob2].grades[grade]
