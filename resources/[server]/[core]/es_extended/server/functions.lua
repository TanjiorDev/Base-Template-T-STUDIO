...
function ESX.GetNumPlayers(key, val)
    ...
    if type(val) == "table" then
        ...
        if key == "job2" then
            for _, v in ipairs(val) do
                numPlayers[v] = (Core.Jobs2PlayerCount[v] or 0)
            end
            return numPlayers
        end
    ...
    if key == "job2" then
        return (Core.Jobs2PlayerCount[val] or 0)
    end
...
---@return nil
---@description Ajout du Job2 (Nexora)
function ESX.RefreshJobs2()
    Core.Jobs2Loaded = false
    ...
    for _, v in pairs(Jobs2) do
        if ESX.Table.SizeOf(v.grades) == 0 then
            Jobs2[v.name] = nil
            print(('[^3WARNING^7] Ignoring job2 ^5"%s"^0 due to no job2 grades found'):format(v.name))
        end
    end

    if not Jobs2 then
        -- Fallback data, if no jobs exist
        ESX.Jobs2["unemployed"] = { name = "unemployed", label = "Unemployed", whitelisted = false, grades = { ["0"] = { grade = 0, name = "unemployed", label = "Unemployed", salary = 200, skin_male = {}, skin_female = {} } } }
    else
        ESX.Jobs2 = Jobs2
    end

    TriggerEvent("esx:jobs2Refreshed")
    Core.Jobs2Loaded = true
end
