...
local function onPlayerDropped(playerId, reason, cb)
    ...
    local currentJob2 = Core.Jobs2PlayerCount[job2]
    Core.Jobs2PlayerCount[job2] = ((currentJob2 and currentJob2 > 0) and currentJob2 or 1) - 1

    GlobalState[("%s:count"):format(job2)] = Core.Jobs2PlayerCount[job2]
...
if Config.Multichar then
    AddEventHandler("esx:onPlayerJoined", function(src, char, data)
        while not ESX.Jobs or not next(ESX.Jobs)
        or not ESX.Jobs2 or not next(ESX.Jobs2) do
            Wait(50)
        end
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    -- Job2
    local job2, grade = result.job2, tostring(result.job2_grade)

    if not ESX.DoesJob2Exist(job2, grade) then
        print(("[^3WARNING^7] Ignoring invalid job2 for ^5%s^7 [job2: ^5%s^7, grade: ^5%s^7]"):format(identifier, job2, grade))
        job2, grade = "unemployed", "0"
    end

    if not ESX.Jobs2 then
        return nil, nil
    end
    
    local job2Object = ESX.Jobs2[job2]
    if not job2Object then
        return nil, nil
    end
    
    local gradeObject = job2Object.grades and job2Object.grades[tostring(grade)] or nil
...
AddEventHandler("esx:playerLoaded", function(_, xPlayer, isNew)
    ...
    Core.Jobs2PlayerCount[job2] = (Core.Jobs2PlayerCount[job2] or 0) + 1
    GlobalState[job2Key] = Core.Jobs2PlayerCount[job2]
    if isNew then
        Player(xPlayer.source).state:set('isNew', true, false)
    end
end)
...
AddEventHandler("esx:setJob2", function(_, job2, lastJob2)
    local lastJob2Key = ("%s:count"):format(lastJob2.name)
    local job2Key = ("%s:count"):format(job2.name)
    local currentLastJob2 = Core.Jobs2PlayerCount[lastJob2.name]

    Core.Jobs2PlayerCount[lastJob2.name] = ((currentLastJob2 and currentLastJob2 > 0) and currentLastJob2 or 1) - 1
    Core.Jobs2PlayerCount[job2.name] = (Core.Jobs2PlayerCount[job2.name] or 0) + 1

    GlobalState[lastJob2Key] = Core.Jobs2PlayerCount[lastJob2.name]
    GlobalState[job2Key] = Core.Jobs2PlayerCount[job2.name]
end)
