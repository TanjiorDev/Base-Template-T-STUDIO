...
ESX.RegisterCommand(
    ...
    "admin",
    function(xPlayer, args, showError)
        if not ESX.DoesJob2Exist(args.job2, args.grade) then
...
ESX.RegisterCommand("info", { "user", "admin" }, function(xPlayer)
    ...
    print(("^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job: ^5%s^0 | ^2Job2: ^5%s^0"):format(xPlayer.source, xPlayer.getName(), xPlayer.getGroup(), job, job2))
end, false)
