...
local function setPlayerModel(model, force)
    if type(model) == "string" then model = joaat(model) end

    if IsModelInCdimage(model) then
        local needsModelChange = GetEntityModel(cache.ped) ~= model
        if needsModelChange or force then
            RequestModel(model)
...
local function setPlayerAppearance(appearance, force)
    if appearance then
        setPlayerModel(appearance.model, force)
