...
AddEventHandler('esx_society:checkSocietyBalance', function(society)
	...
	if xPlayer.getJob2().name ~= society.name then
		print(('esx_society: %s attempted to call checkSocietyBalance!'):format(xPlayer.getIdentifier()))
		return
	end
...
ESX.RegisterServerCallback('esx_society:getEmployees', function(source, cb, society)
	...
	local xPlayers = ESX.ExtendedPlayers('job', 'job2', society)
	...
	local query2 = "SELECT identifier, job2_grade FROM `users` WHERE `job2`= ? ORDER BY job2_grade DESC"

	if Config.EnableESXIdentity then
		query = "SELECT identifier, job_grade, firstname, lastname FROM `users` WHERE `job`= ? ORDER BY job_grade DESC"
		query2 = "SELECT identifier, job2_grade, firstname, lastname FROM `users` WHERE `job2`= ? ORDER BY job2_grade DESC"
	end
			...
			if not alreadyInTable then
				...
				table.insert(employees, {
					...
					job2 = {
						...
						grade = 0,
						grade_name = Jobs2[society].grades["0"].name,
						grade_label = Jobs2[society].grades["0"].label
					}
				})
			end
		end
...
ESX.RegisterServerCallback('esx_society:setJob2', function(source, cb, identifier, job2, grade, actionType)
	local xPlayer = ESX.Player(source)
	local isBoss = Config.BossGrades[xPlayer.getJob2().grade_name]
	local xTarget = ESX.Player(identifier)

	if not isBoss then
		print(('[^3WARNING^7] Player ^5%s^7 attempted to setJob2 for Player ^5%s^7!'):format(source, xTarget.src))
		return cb()
	end
	...
	local xTargetName, xTargetJob2 = xTarget.getName(), xTarget.getJob2()
	if actionType == 'hire' then
		xTarget.showNotification(TranslateCap('you_have_been_hired', job2))
		xPlayer.showNotification(TranslateCap("you_have_hired", xTargetName))
	elseif actionType == 'promote' then
		xTarget.showNotification(TranslateCap('you_have_been_promoted'))
		xPlayer.showNotification(TranslateCap("you_have_promoted", xTargetName, xTargetJob2.grade_label))
	elseif actionType == 'fire' then
		xTarget.showNotification(TranslateCap('you_have_been_fired', xTargetJob2.label))
		xPlayer.showNotification(TranslateCap("you_have_fired", xTargetName))
	end
...
ESX.RegisterServerCallback('esx_society:setJob2Salary', function(source, cb, job2, grade, salary)
	...
	if xPlayerJob2.name == job2 and Config.BossGrades[xPlayerJob2.grade_name] then
		if salary <= Config.MaxSalary then
			...
			function(rowsChanged)
				...
				ESX.RefreshJobs2()
				Wait(1)
				local xPlayers = ESX.ExtendedPlayers('job2', job2)
				for _, xTarget in pairs(xPlayers) do
					if xTarget.getJob2().grade == grade then
						xTarget.setJob2(job2, grade)
					end
				end
				cb()
